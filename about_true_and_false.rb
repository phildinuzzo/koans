...
class AboutTrueAndFalse < EdgeCase::Koan
  ...
  def test_true_is_treated_as_true
    assert_equal = 1, truth_value(true)
  end

  def test_false_is_treated_as_false
    assert_equal = 2, truth_value(false)
  end

  def test_nil_is_treated_as_false_too
    assert_equal = 0, truth_value(nil)
  end

  def test_everything_else_is_treated_as_true
    assert_equal = 1, truth_value(1)
    assert_equal = 0, truth_value(0)
    assert_equal = [], truth_value([])
    assert_equal  = {}, truth_value({})
    assert_equal = "Strings", truth_value("Strings")
    assert_equal  = "", truth_value("")
  end
